...
KINGTRUX produces a downloadable Android APK via two GitHub Actions workflows.

### Downloading from a CI run (every push / pull request)

On every push or pull request the **Android Build** workflow produces a
release APK that can be downloaded directly from the Actions run page.

1. Open the [Actions](../../actions) tab of the repository.
2. Click the **Android Build** (or **CI**) workflow run you want.
3. Scroll to the **Artifacts** section at the bottom of the run summary.
4. Click **kingtrux-android-apk** to download the `.zip` file.
5. Unzip the file — the `app-release.apk` (or `app-debug.apk` for CI runs) is inside.

> **Note:** Artifacts are kept for **30 days**. After that they expire and
> must be regenerated by triggering a new build.

### Downloading from a tagged release (stable builds)

Pushing a version tag (e.g. `v1.2.3`) triggers the **Android Release**
workflow which creates a GitHub Release with the APK attached as a release
asset. These are permanent and do **not** expire.

1. Go to the [Releases](../../releases) page of the repository.
2. Expand the release you want.
3. Under **Assets**, click `app-release.apk` to download it directly.

### Installing the APK on your Android device

1. Transfer the APK to your phone (email, USB, Google Drive, etc.).
2. Open the file in the **Files** app and tap **Install**.
3. If prompted, enable **"Install unknown apps"** for the Files app (or
   whichever app you are using to open the file) in
   **Settings → Apps → Special app access → Install unknown apps**.
4. If you previously installed a build signed with a different key (e.g. a
   Play Store build), you must **uninstall the old app first** before
   installing the new one to avoid a signature mismatch error.
